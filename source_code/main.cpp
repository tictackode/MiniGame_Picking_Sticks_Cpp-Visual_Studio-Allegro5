...
void initStickPosition()
{
	...
	wooden_stick.x = rand() % gameScreen.NUMBER_OF_BLOCKS;
	wooden_stick.y = rand() % gameScreen.NUMBER_OF_BLOCKS;
}
...
int main()
{
	...
	al_set_window_title(gameScreen.display,"tictacShooter >>> an opensource spaceship shooter game");
	...
	while (running) 
	{
		...
		//Slows down the player speed movement
		if (ciclos % 5 == 0)
		{
			keyPressed=false;
		}
	...
	al_destroy_bitmap(wooden_stick.image);
